...
export function createGuide(input, {onClose, onControls} = {}) {
  ...
  dialog.refresh = () => {
    ...
    dialog.body.innerHTML = `<p class="dialog-intro">Start with Practice flight. You are already airborne, with no enemies or mission clock. Get comfortable, then enter the campaign.</p>
      ...
      <div class="info-callout"><b>Plan your next move.</b> ${key('map')} opens the tactical map with your position, current objective, airfield course and received radio calls. The world pauses while you read. You can also open it from Pause.</div>
