...
export class ControlsMenu {
  ...
  _settingsHtml() {
    ...
    h += slider("weaponsVol", "WEAPONS / EFFECTS VOLUME", 0, 1, 0.05, s.weaponsVol, Math.round(s.weaponsVol * 100) + "%", audioOn);
    h += slider("uiVol", "WARNINGS / RADIO VOLUME", 0, 1, 0.05, s.uiVol, Math.round(s.uiVol * 100) + "%", audioOn);
