...
export class ControlsMenu {
  ...
  _graphicsStatusHtml() {
    ...
    return `<div class="graphics-status" role="status"><div><strong>${next!==state.tier ? `${esc(next)} is ready for your next flight` : `${esc(state.tier)} is running`}</strong><p>${next!==state.tier ? 'Restart applies the full preset. Your unfinished flight will start over.' : 'Field of view and resolution appear when you resume.'}${options.tier==='AUTO'&&state.recommendedTier ? ` Auto measured this scene and recommends ${esc(state.recommendedTier)}.` : ''}</p></div>${next!==state.tier ? '<button type="button" class="setup-button" data-review-restart>Review restart ↗</button>' : ''}</div>`;
  }
