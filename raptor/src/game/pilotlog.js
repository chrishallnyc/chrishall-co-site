import { CAMPAIGN, loadAuth, isUnlocked, loadSortie } from '../campaign/authored.js';
...
export class PilotLog extends GameDialog {
  ...
  render() {
    ...
    this.body.innerHTML=`<div class="log-summary"><div><strong>${p.completed}<span> / ${p.total}</span></strong><p>missions completed</p></div><div class="log-progress"><progress max="${p.total}" value="${p.completed}" aria-label="Campaign completion"></progress><p>${p.next?`Next up: ${escapeHTML(this.catalog.find(c=>c.id===p.next.id)?.title || p.next.id)}`:'Campaign complete. Replay any mission.'}</p></div><span class="save-note">Local progress</span></div>
      <div class="log-filters" role="group" aria-label="Filter missions by region">${[['ALL','All regions'],...Object.entries(FRONTS).map(([id,f])=>[id,f.name])].map(([id,name])=>`<button type="button" class="ui-button ${this.filter===id?'selected':''}" aria-pressed="${this.filter===id}" data-filter="${id}">${name}</button>`).join('')}</div>
      ...
      <section class="operation-ledger"><h3>Ongoing operations</h3><p class="section-description">Each region has its own persistent front line, separate from the campaign.</p><div class="operation-grid">${Object.entries(FRONTS).map(([id,f])=>{const s=operationSummary(id);return `<article><p class="eyebrow">${f.name}</p><strong>${s.frontKm>0?'+':''}${s.frontKm.toFixed(1)} km</strong><p>${s.sortieIndex} sorties flown · ${s.status==='live'?'In progress':s.status==='won'?'Won':'Lost'}</p><small>Next: ${escapeHTML(missionTypeLabel(s.nextType))} · ${escapeHTML(s.nextZoneName)}</small></article>`;}).join('')}</div></section>`;
