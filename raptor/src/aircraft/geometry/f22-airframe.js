...
const cowl = z => {
  const s = cowlStations(z), wingY = wingUpperAt(s.upperWidth, z);
  if (wingY !== null) {
...
function centralTop(x, z) {
  ...
  return s.upperEdge + (s.crown - s.upperEdge) * Math.pow(Math.max(0, 1 - t ** n), 1 / n);
}
...
function roofSection(t, z) {
  ...
  return t < ridge ?
    lerp(a.upperEdge, peak, Math.sin(t / ridge * Math.PI / 2)) :
    lerp(peak, b.upperEdge, (1 - Math.cos((t - ridge) / (1 - ridge) * Math.PI)) / 2);
}
...
export function buildAirframeGeometry({ quality = 'high', longitudinal, transverse, cutOpenings = true } = {}) {
  ...
  const addPair = (name, evaluate, options = {}, material = 'skin') => {
    const right = surfacePatch(evaluate, { uSegments: transverse, vSegments: longitudinal, ...options, name: `${name}R` });
  ...
  for (const [name, front, back, cockpit, segments] of [
    ...
    addPair(name, (u, v) => {
      ...
      const t = cockpit ? u : Math.sin(u * Math.PI / 2);
  ...
  for (let edge = 0; edge < 4; edge++) {
    ...
    addPair(`intakeLip${edge}`, (u, v) => {
      const p = rim(u);
      return [lerp(p[0], centre[0] + (p[0] - centre[0]) * .962, v),
        lerp(p[1], centre[1] + (p[1] - centre[1]) * .962, v), p[2] + v * .025];
    }, { uSegments: Math.max(2, Math.round(12*small)), vSegments: 1, uv: sideUV }, 'lip');
    addPair(`intakeDuct${edge}`, (u, v) => {
      const p = rim(u), shrink = lerp(.962, .64, v);
      return [centre[0] + (p[0] - centre[0]) * shrink + .06 * v,
        centre[1] + (p[1] - centre[1]) * shrink + .14 * v * v,
        p[2] + .025 + 2.45 * v];
    }, { uSegments: Math.max(2, Math.round(12*small)), vSegments: Math.max(3, Math.round(12*small)), uv: innerUV }, 'duct');
  }
  ...
  const end = corners.map(p => [centre[0] + (p[0] - centre[0]) * .64 + .06,
    centre[1] + (p[1] - centre[1]) * .64 + .14, p[2] + 2.475]);
