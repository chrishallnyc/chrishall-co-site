...
assert.match(await page.locator('#readiness-comfort').textContent(),/Muted/);await page.locator('[data-open="settings"]').first().click();await page.locator('[data-tab="audio"]').click();assert.equal(await page.locator('#setup-muted').isChecked(),true);await page.locator('#setup-muted').uncheck();await page.keyboard.press('Escape');await page.reload({waitUntil:'networkidle'});assert.doesNotMatch(await page.locator('#readiness-comfort').textContent(),/Muted/);
});
