...
export class TerrainMap {
  ...
  _style() {
    ...
    return {
      ...
      layers: [
        ...
        { id: 'elevation', type: 'color-relief', source: 'shading', layout: { visibility: 'none' }, paint: { 'color-relief-color': ELEVATION_COLORS } },
        { id: 'imagery', type: 'raster', source: 'imagery', paint: {
          'raster-saturation': -0.22, 'raster-contrast': 0.08, 'raster-brightness-max': 0.87,
  ...
  setState(state, feature) {
    ...
    this.map.getSource('selected-state').setData(boundary || EMPTY);
    if (boundary) {
      const polygons = boundary.geometry.type === 'MultiPolygon' ? boundary.geometry.coordinates : [boundary.geometry.coordinates];
      // A complete world polygon with state exteriors punched out preserves real shorelines.
      // Western longitudes also cover Alaska's unwrapped Aleutian Islands.
      const world = [[-360, -85.05], [0, -85.05], [0, 85.05], [-360, 85.05], [-360, -85.05]];
      this.map.getSource('outside-state').setData({ type: 'Feature', properties: {}, geometry: {
        type: 'Polygon', coordinates: [world, ...polygons.map(polygon => polygon[0])],
      } });
    } else this.map.getSource('outside-state').setData(EMPTY);
    this.resetView();
  }
  ...
  focusPlace(place, options = {}) {
    ...
    const zoom = options.zoom ?? place.zoom ?? (place.type === 'city' ? 12.2 : place.type === 'feature' ? 10.8 : 12.8);
  ...
  setSurface(mode) {
    ...
    this.map.setLayoutProperty('elevation', 'visibility', mode === 'natural' ? 'none' : 'visible');
